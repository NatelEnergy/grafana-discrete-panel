      ...
      execute: function() {
        ...
        DiscretePanelCtrl = (function(_super) {
            ...
            for (var i = 0; i < this.panel.valueMaps.length; i++) {
              ...
              if (this.panel.valueMaps[i].text === val) {
                if (this.colorMap.hasOwnProperty(this.panel.valueMaps[i].value)) {
                  this._colorsPaleteCash[val] = this.colorMap[
                    this.panel.valueMaps[i].value
                  ];
                } else {
                      ...
                      if (colorSwitch.length === 0) {
                        ...
                        for (var i = 0; i < colorSwitch.length; i++) {
                          if (colorSwitch[i].colorText === info.val) {
                            v = {
                              text: info.val,
                              color: _this.getColor(colorSwitch[i].colorValue),
                            };
                          } else {
                            v = {text: info.val, color: _this.getColor(info.val)};
                          }
          ...
          DiscretePanelCtrl.prototype._updateCanvasSize = function() {
            ...
            jquery_1
              .default(this.canvas)
              .css('width', this._renderDimensions.width + 'px');
            jquery_1
              .default(this.canvas)
              .css('height', this._renderDimensions.height + 'px');
            ...
            lodash_1.default.forEach(this.data, function(metric, i) {
              ...
              for (var j = 0; j < rowObj.positions.length; j++) {
                ...
                ctx.fillRect(
                  ...
                  matrix[i].y,
                  nextX - currentX,
                  _this._renderDimensions.rowHeight
                );
          ...
          DiscretePanelCtrl.prototype._renderLabels = function() {
            ...
            var rowHeight = this._renderDimensions.rowHeight;
            lodash_1.default.forEach(this.data, function(metric, i) {
              var _a = _this._renderDimensions.matrix[i],
                y = _a.y,
                ...
                for (var j = 0; j < positions.length; j++) {
                  if (positions[j] <= _this.mouse.position.x) {
                    if (
                      ...
                      positions[j + 1] >= _this.mouse.position.x
                    ) {
              ...
              if (_this.panel.writeLastValue && positions.length > 0) {
                ...
                if (xval > hoverTextEnd) {
                  ctx.fillText(
                    val,
                    _this._renderDimensions.width - offset,
                    labelPositionLastValue
                  );
                  maxTextSpot =
                    _this._renderDimensions.width - ctx.measureText(val).width - 10;
                }
              }
          ...
          DiscretePanelCtrl.prototype._renderTimeAxis = function() {
            ...
            ctx.font =
              this.panel.textSizeTime + ' "Open Sans", Helvetica, Arial, sans-serif';
            ...
            var min = lodash_1.default.isUndefined(this.range.from)
              ? null
              : this.range.from.valueOf();
            var max = lodash_1.default.isUndefined(this.range.to)
              ? null
            ...
            var timeFormat = this.timeSrv.graphTimeFormat(
              timeResolution / 1000,
              min,
              max
            );
          ...
          DiscretePanelCtrl.prototype.initI18n = function() {
            if (this.dashboard.panelLanguage) {
              ...
              for (var _b = 0, _c = this.panel.rangeMaps; _b < _c.length; _b++) {
                var rangeMap = _c[_b];
                rangeMap.text = i18n_1.getI18n(
                  rangeMap,
                  'text',
                  this.dashboard.panelLanguage
                );
              }
              this.panel.FontSize = i18n_1.getI18n(
                this.panel,
                'FontSize',
                this.dashboard.panelLanguage
              );
              this.panel.FontSizeValue = i18n_1.getI18n(
                this.panel,
                'FontSizeValue',
                this.dashboard.panelLanguage
              );
